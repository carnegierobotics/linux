...
static int ambarella_i2s_probe(struct platform_device *pdev)
{
	...
	u32 clk_au_enable = 0, scr_offset;
	...
	clk_au_enable = !!of_find_property(np, "amb,clk-au-enable", NULL);
	if (clk_au_enable) {
		scr_regmap = syscon_regmap_lookup_by_phandle(np, "amb,scr-regmap");
		if (IS_ERR(scr_regmap)) {
			dev_err(&pdev->dev, "no regmap!\n");
			rval = PTR_ERR(scr_regmap);
			return rval;
		}

		if (of_property_read_u32_index(np, "amb,scr-regmap", 1, &scr_offset)) {
			dev_err(&pdev->dev, "couldn't get clk_au pad offset\n");
			rval= -EINVAL;
			return rval;
		}
		rval = regmap_update_bits(scr_regmap, scr_offset, BIT(0), 0);
		if (rval < 0)
			return rval;
	}
