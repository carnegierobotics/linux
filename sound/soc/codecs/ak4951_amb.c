		...
		struct snd_kcontrol *kcontrol, int event) //CONFIG_LINF
{
	...
	reg = snd_soc_component_read(component, AK4951_01_POWER_MANAGEMENT2);
	nLOSEL = (0x1 & reg);
		...
		unsigned int freq, int dir)
{
	...
	pll = snd_soc_component_read(component, AK4951_05_MODE_CONTROL1);
	akdbgprt("\t[AK4951] pll valuse is 0x%x\n",pll);
	pll &=(~0xF0);
	pllpwr = snd_soc_component_read(component,AK4951_01_POWER_MANAGEMENT2);
...
static int ak4951_set_dai_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	mode = snd_soc_component_read(component, AK4951_01_POWER_MANAGEMENT2);
	format = snd_soc_component_read(component, AK4951_05_MODE_CONTROL1);
	format &= ~AK4951_DIF;
			...
			u16 mask, u16 value)
{
	...
	if ( (mask == 0) || (mask == 0xFF) ) {
		...
		olddata = snd_soc_component_read(component, reg);
		...
		enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_STANDBY:
		reg = snd_soc_component_read(component, AK4951_00_POWER_MANAGEMENT1);	// * for AK4951
...
static int ak4951_suspend(struct snd_soc_component *component)
{
	...
	for(i = 0; i < 7; i++)
		ak4951->reg_cache[i] = snd_soc_component_read(component, i);
