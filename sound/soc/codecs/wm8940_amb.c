		...
		int source, unsigned int freq_in, unsigned int freq_out)
{
	...
	snd_soc_component_write(component, WM8940_PLLK1, (pll_div.k >> 18) & 0x3f);
				 ...
				 int clk_id, unsigned int freq, int dir)
{
	...
	case WM8940_MCLKIN:
		/* Clock CODEC directly from MCLK */
		reg = snd_soc_component_read(component, WM8940_CLOCK) & 0x0fe;
		snd_soc_component_write(component, WM8940_CLOCK, reg);
...
static int wm8940_suspend(struct snd_soc_component *component)
{
	return wm8940_set_bias_level(component, SND_SOC_BIAS_OFF);
}

static int wm8940_resume(struct snd_soc_component *component)
{
	wm8940_set_bias_level(component, SND_SOC_BIAS_STANDBY);
	return 0;
}
...
static int wm8940_probe(struct snd_soc_component *component)
{
	...
	snd_soc_component_write(component, WM8940_POWER1, 0x180);

	/************************ RECORD PATH SETTINGS *******************************/
	snd_soc_component_write(component, WM8940_CLOCK, 0x0100);	/*set MCLK, MCLKDIV 1, BCLKDIV 1, BCLK and frame clock are inputs*/
	snd_soc_component_write(component, WM8940_GPIO, 0x0000);
	snd_soc_component_write(component, WM8940_ADDCNTRL, 0x0000);
	snd_soc_component_write(component, WM8940_INPUTCTL, 0x0107);
	snd_soc_component_write(component, WM8940_PGAGAIN, 0x003d);	/*set PGA gain 30db*/
	snd_soc_component_write(component, WM8940_ADCBOOST, 0x0155);	/*set PGB2BOOST, MICP2BOOSTVOL gain*/
	snd_soc_component_write(component, WM8940_ADC, 0x01c0);	/*Enable high pass filter 396HZ under 48k, select audio mode*/
	snd_soc_component_write(component, WM8940_ADCVOL, 0x00ff);	/*Set ADC vol, 0db*/
	snd_soc_component_write(component, WM8940_ALC1, 0x0138);	/*Enable ALC, set max gain +35.25db and min gain -12db*/
	snd_soc_component_write(component, WM8940_ALC2, 0x0007);	/*set min alc target gain -12db and hold time 0ms*/
	snd_soc_component_write(component, WM8940_ALC3, 0x0042);	/*ALC mode, DCY 384ms, ATK 24ms*/
	snd_soc_component_write(component, WM8940_ALC4, 0x0000);	/*ZC OFF*/
	snd_soc_component_write(component, WM8940_NOISEGATE, 0x000f); /*Enable nosie gate and set threshold -81db*/
	snd_soc_component_write(component, WM8940_NOTCH8, 0x00c1); /*Set low pass filter from 15khz*/

	/************************ PLAYBACK PATH SETTINGS *******************************/
	snd_soc_component_write(component, WM8940_DAC, 0x0000);		/*Disable soft mute, Disable DAC auto mute, no inversion*/
	snd_soc_component_write(component, WM8940_DACVOL, 0x00ff);	/*Set DACVOL 0dB*/
	snd_soc_component_write(component, WM8940_DACLIM1, 0x0000);	/*Disable DAC Limiter*/
	snd_soc_component_write(component, WM8940_DACLIM2, 0x000c);	/*Set DAC boost 12dB*/
	snd_soc_component_write(component, WM8940_SPKMIX, 0x0001);	/*select output DAC to speaker mixer*/
	snd_soc_component_write(component, WM8940_SPKVOL, 0x003f);	/*Eable speaker out and set speaker volume +6dB*/
	snd_soc_component_write(component, WM8940_MONOMIX, 0x0000); /*Not select DAC to mono*/

	return ret;
}

static void wm8940_remove(struct snd_soc_component *component)
{
	wm8940_set_bias_level(component, SND_SOC_BIAS_OFF);
}
