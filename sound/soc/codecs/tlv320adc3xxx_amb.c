 ...
 *----------------------------------------------------------------------------
 */
int adc3xxx_change_page(struct snd_soc_component *component, u8 new_page)
{
	struct adc3xxx_priv *adc3xxx = snd_soc_component_get_drvdata(component);

	if (i2c_smbus_write_byte_data(adc3xxx->i2c, 0x0, new_page) < 0) {
 ...
 *----------------------------------------------------------------------------
 */
static inline void adc3xxx_write_reg_cache(struct snd_soc_component *component,
					   unsigned int reg, unsigned int value)
{
	struct adc3xxx_priv *adc3xxx = snd_soc_component_get_drvdata(component);
	u8 *cache = adc3xxx->reg_cache;
 ...
 *----------------------------------------------------------------------------
 */
static inline unsigned int adc3xxx_read_reg_cache(struct snd_soc_component *component,
						  unsigned int reg)
{
	struct adc3xxx_priv *adc3xxx = snd_soc_component_get_drvdata(component);
	u8 *cache = adc3xxx->reg_cache;
 ...
 *----------------------------------------------------------------------------
 */
int adc3xxx_write(struct snd_soc_component *component, unsigned int reg,
		  unsigned int value)
{
	struct adc3xxx_priv *adc3xxx = snd_soc_component_get_drvdata(component);
	u8 data[2];
	...
	if (adc3xxx->page_no != page) {
		adc3xxx_change_page(component, page);
	}
	...
	if (i2c_smbus_write_byte_data(adc3xxx->i2c, data[0], data[1]) < 0) {
	...
	if ((page == 0) || (page == 1)) {
		adc3xxx_write_reg_cache(component, reg, data[1]);
	}
	return 0;
}
 ...
 *----------------------------------------------------------------------------
 */
static unsigned int adc3xxx_read(struct snd_soc_component *component, unsigned int reg)
{
	struct adc3xxx_priv *adc3xxx = snd_soc_component_get_drvdata(component);
	u8 value, page;
	...
	/*  read register value */
	value = i2c_smbus_read_word_data(adc3xxx->i2c, reg);
	if (value < 0) {
	...
	/* Update register cache */
	if ((page == 0) || (page == 1)) {
		adc3xxx_write_reg_cache(component, reg, value);
	}
	return value;
}
				     ...
				     struct snd_ctl_elem_value *ucontrol)
{
	...
	reg = adc3xxx_read_reg_cache(component, LADC_VOL) & (~0x7F);
	adc3xxx_write(component, LADC_VOL, reg | (val1 << 0));
	reg = adc3xxx_read_reg_cache(component, RADC_VOL) & (~0x7F);
	adc3xxx_write(component, RADC_VOL, reg | (val2 << 0));
				     ...
				     struct snd_ctl_elem_value *ucontrol)
{
	...
	val1 = adc3xxx_read_reg_cache(component, LADC_VOL) & (0x7F);
	if ((val1 >= 0) && (val1 <= 40)) {
		val1 = val1 + ADC_POS_VOL;
	...
	val2 = adc3xxx_read_reg_cache(component, RADC_VOL) & (0x7F);
	if ((val2 >= 0) && (val2 <= 40)) {
		val2 = val2 + ADC_POS_VOL;
			     ...
			     struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	data =
	    adc3xxx_read_reg_cache(component, INTERFACE_CTRL_1) & (~WLENGTH_MASK);
	...
	adc3xxx_write(component, INTERFACE_CTRL_1, data);
	...
	/* P & R values */
	adc3xxx_write(component, PLL_PROG_PR,
		      (adc3xxx_divs[i].pll_p << PLLP_SHIFT) | (adc3xxx_divs[i].
							       pll_r <<
							       PLLR_SHIFT));
	/* J value */
	adc3xxx_write(component, PLL_PROG_J, adc3xxx_divs[i].pll_j & PLLJ_MASK);
	/* D value */
	adc3xxx_write(component, PLL_PROG_D_LSB,
		      adc3xxx_divs[i].pll_d & PLLD_LSB_MASK);
	adc3xxx_write(component, PLL_PROG_D_MSB,
		      (adc3xxx_divs[i].pll_d >> 8) & PLLD_MSB_MASK);
	/* NADC */
	adc3xxx_write(component, ADC_NADC, adc3xxx_divs[i].nadc & NADC_MASK);
	/* MADC */
	adc3xxx_write(component, ADC_MADC, adc3xxx_divs[i].madc & MADC_MASK);
	/* AOSR */
	adc3xxx_write(component, ADC_AOSR, adc3xxx_divs[i].aosr & AOSR_MASK);
	/* BDIV N Value */
	adc3xxx_write(component, BCLK_N_DIV, bdiv & BDIV_MASK);
	msleep(10);
...
static int adc3xxx_set_dai_fmt(struct snd_soc_dai *codec_dai, unsigned int fmt)
{
	...
	iface_reg =
	    adc3xxx_read_reg_cache(component, INTERFACE_CTRL_1) & (~FMT_MASK);
	...
	case SND_SOC_DAIFMT_CBS_CFM: //new case..just for debugging
		...
		adc3xxx->master = 0;
		/* BCLK by component ie BCLK output */
	...
	/* set iface */
	adc3xxx_write(component, INTERFACE_CTRL_1, iface_reg);
	return 0;
}
				  ...
				  enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_ON:
		if (adc3xxx->master) {
			/* Enable pll */
			reg = adc3xxx_read_reg_cache(component, PLL_PROG_PR);
			adc3xxx_write(component, PLL_PROG_PR, reg | ENABLE_PLL);

			/* 10msec delay needed after PLL power-up */
			mdelay(10);

			/* Switch on NADC Divider */
			reg = adc3xxx_read_reg_cache(component, ADC_NADC);
			adc3xxx_write(component, ADC_NADC, reg | ENABLE_NADC);

			/* Switch on MADC Divider */
			reg = adc3xxx_read_reg_cache(component, ADC_MADC);
			adc3xxx_write(component, ADC_MADC, reg | ENABLE_MADC);

			/* Switch on BCLK_N Divider */
			reg = adc3xxx_read_reg_cache(component, BCLK_N_DIV);
			adc3xxx_write(component, BCLK_N_DIV, reg | ENABLE_BCLK);
		} else { //new
			/* Enable pll */
			reg = adc3xxx_read_reg_cache(component, PLL_PROG_PR);
			adc3xxx_write(component, PLL_PROG_PR, reg | ENABLE_PLL);
	...
	case SND_SOC_BIAS_OFF:
		...
		reg = adc3xxx_read_reg_cache(component, ADC_DIGITAL);
		adc3xxx_write(component, ADC_DIGITAL,
...
static int adc3xxx_init(struct snd_soc_component *component)
{
	...
	if (!adc3xxx->reg_cache)
		return -ENOMEM;

	/* Select Page 0 */
	adc3xxx_write(component, PAGE_SELECT, 0);
	/* Issue software reset to adc3xxx */
	adc3xxx_write(component, RESET, SOFT_RESET);
	...
	for (i = 0;
	     ...
	     i++) {
		adc3xxx_write(component, adc3xxx_reg_init[i].reg_offset,
...
static int adc3xxx_probe(struct snd_soc_component *component)
{
	...
	adc3xxx = (struct adc3xxx_priv *)snd_soc_component_get_drvdata(component);
...
const struct snd_soc_component_driver soc_codec_dev_adc3xxx = {
	.probe			= adc3xxx_probe,
