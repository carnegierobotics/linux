...
static int ambfb_request_frame_buffer(struct fb_info *info)
{
	int rval = 0;

	rval = ambfb_notifier_call_chain(info, AMBFB_EVENT_REQUEST_FB, NULL);
	if (rval == NOTIFY_DONE) {
		rval = -ENOMEM;
...
static int ambfb_release_frame_buffer(struct fb_info *info)
{
	int rval = 0;

	rval = ambfb_notifier_call_chain(info, AMBFB_EVENT_RELEASE_FB, NULL);
	if (rval == NOTIFY_DONE) {
		rval = -ENOMEM;
	} else if (notifier_to_errno(rval) < 0) {
		rval = notifier_to_errno(rval);
...
static int ambfb_open(struct fb_info *info, int user)
{
	...
	if (count == 0)
		dev_warn(par->dev, "Framebuffer has already been opened!\n");

	if (count == 0 || atomic_inc_and_test(&par->use_count)) {
...
static int ambfb_parse_dt(struct ambfb_par *par)
{
	...
	rval = of_property_read_u32(np, "fb_from_ambcma", &fb_from_ambcma);
	if (rval < 0) {
		fb_from_ambcma = 0;
	}
	fb_from_ambcma = !!fb_from_ambcma;
...
static int ambfb_probe(struct platform_device *pdev)
{
	...
	if (!fb_from_ambcma) {
		...
		if (ambfb_request_frame_buffer(info) < 0) {
			dev_err(&pdev->dev,
				"ambfb request buffer failed, smem_len is 0x%x!\n",
				fix->smem_len);
			rval = -ENOMEM;
			goto exit0;
		}
	...
	dev_info(&pdev->dev, "%dx%d, %s, %d bits per pixel, buf pitch = %d\n",
		 ...
		 par->format->bits_per_pixel, fix->line_length);
...
exit0:
	if (info->screen_base) {
		if (fb_from_ambcma) {
			ambfb_release_frame_buffer(info);
		}
	}
